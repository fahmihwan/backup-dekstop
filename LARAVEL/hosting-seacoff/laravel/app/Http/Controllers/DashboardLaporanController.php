...
class DashboardLaporanController extends Controller
{
    ...
    public function detail_pemesanan($nota)
    {
        $data = DetailOrders::where('nota', $nota)->first();
        $order = Order::select('*')
            ->join('menus', 'orders.menu_id', '=', 'menus.id')
            ->where('detail_orders_id', $data->id)->get();


        return view('admin.pages.laporan.detail_pemesanan', [
            'detail_order' => $data,
            'order' => $order,
        ]);
    }
