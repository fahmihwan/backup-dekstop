...
@section('container')
    ...
    <div class="row float-end py-2 mt-5">
        <form action="/admin/event/{{ $event->id }}" method="post" id="submit-form" hidden>
            {{-- <form action="/admin/item/{{ $menu[0]->id }}" method="post" id="submit-form" hidden> --}}
            @method('delete')
            @csrf
        </form>
        <p class="me-5 pb-5"> Tekan <a href="/admin" id='delete-item'>Hapus</a>, jika ingin menghapus event
            <b>{{ $event->title }} </b> -
            {{ $event->date }} ?
        </p>
    </div>
