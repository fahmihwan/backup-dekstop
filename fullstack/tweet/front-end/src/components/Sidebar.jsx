import axios from "axios";
import React from "react";
import { NavLink } from "react-router-dom";
import HttpCommon from "../http-common";
import { useLogoutUserMutation } from "../redux/features/authApi";

const Sidebar = () => {
    const [logout, { data, isLoading, error, isError, isSuccess }] = useLogoutUserMutation();

    const handleLogout = async () => {
        await logout()
            .then((res) => {
                console.log(res);
                localStorage.clear();
            })
            .catch((err) => console.log(err));
    };
    return (
        <div className="w-3/12  relative ">
            <div className="sticky top-0 border  border-gray-700 h-screen">
                <section className="px-10 py-5 ">
                    <div className="w-10 mb-5">
                        <TwitterSvg />
                    </div>
                    <ul>
                        <li className="mb-5">
                            <NavLink className="text-2xl font-light" to="/">
                                Home
                            </NavLink>
                        </li>
                        <li className="mb-5">
                            <NavLink className="text-2xl font-light" to="/profile">
                                Profile
                            </NavLink>
                        </li>
                    </ul>
                </section>

                <section className="absolute bottom-0 border-t border-gray-700 w-full h-20">
                    <div className="w-full flex h-full items-center">
                        <div className="w-3/12 flex justify-center items-center">
                            <div className=" w-10 h-10 bg-gray-400 text-black rounded-full flex justify-center items-center">
                                <i className="text-2xl fa-regular fa-user"></i>
                            </div>
                        </div>
                        <div className="w-9/12 flex justify-between">
                            <div>
                                <p>akuw</p>
                                <p className="text-gray-500">@usernamenyaa</p>
                            </div>

                            <div className="dropdown dropdown-top dropdown-end">
                                {/* <label tabIndex={0} className="btn m-1">
                                    Click
                                </label> */}
                                <label tabIndex={0} className="px-6 h-8 block  cursor-pointer">
                                    <i className="fa-solid text-gray-500 fa-ellipsis"></i>
                                </label>
                                <ul
                                    tabIndex={0}
                                    className="dropdown-content bg-black menu p-2 shadow border border-gray-700 rounded w-52"
                                >
                                    <li>
                                        <button onClick={handleLogout}>Logout</button>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </section>
            </div>
        </div>
    );
};

export default Sidebar;

const TwitterSvg = () => {
    return (
        <svg
            viewBox="0 0 24 24"
            aria-hidden="true"
            fill="white"
            className=" r-13v1u17 r-4qtqp9 r-yyyyoo r-16y2uox r-8kz0gk r-dnmrzs r-bnwqim r-1plcrui r-lrvibr r-lrsllp"
        >
            <g>
                <path d="M23.643 4.937c-.835.37-1.732.62-2.675.733.962-.576 1.7-1.49 2.048-2.578-.9.534-1.897.922-2.958 1.13-.85-.904-2.06-1.47-3.4-1.47-2.572 0-4.658 2.086-4.658 4.66 0 .364.042.718.12 1.06-3.873-.195-7.304-2.05-9.602-4.868-.4.69-.63 1.49-.63 2.342 0 1.616.823 3.043 2.072 3.878-.764-.025-1.482-.234-2.11-.583v.06c0 2.257 1.605 4.14 3.737 4.568-.392.106-.803.162-1.227.162-.3 0-.593-.028-.877-.082.593 1.85 2.313 3.198 4.352 3.234-1.595 1.25-3.604 1.995-5.786 1.995-.376 0-.747-.022-1.112-.065 2.062 1.323 4.51 2.093 7.14 2.093 8.57 0 13.255-7.098 13.255-13.254 0-.2-.005-.402-.014-.602.91-.658 1.7-1.477 2.323-2.41z"></path>
            </g>
        </svg>
    );
};
